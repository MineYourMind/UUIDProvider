...
public class UUIDProvider extends JavaPlugin {
	...
	public void onEnable() {
    	...
    	try {
			...
			this.getLogger().warning("UUIDProvider won't use a MySQL database! This may affect performance!");
			ds=null;
		}
    	...
    	for(dbScrape d : config.scrapeEntries){
    		try {
				ScrapeDataStore sds = new ScrapeDataStore(this, d.ScrapeName,d.dbUrl, d.dbUsername, d.dbPassword, d.dbTable, d.dbColUsername, d.dbColUUID);
				scrapeDS.put(d.ScrapeName, sds);
			} catch (Exception e) {
				this.getLogger().severe("Error creating scrape datastore : " + e.getMessage());
				e.printStackTrace();
			}
		...
		if (cmd.getName().equalsIgnoreCase("uuidprovider")) {
				...
				if (getServer().getConsoleSender() == sender) {
					if (args.length==1) {
						...
						if(pendingScrapes.containsKey(sender.getName())){
							AsynchScrape as = pendingScrapes.get(sender.getName());
							if(as != null && as.ds != null){
								if(as.state == true && as.failure == false){
									sender.sendMessage("§2[UUID§aProvider§2] Scrape finished successfully - Scraped "+as.ds.Count+" player records!");
									pendingScrapes.remove(sender.getName());
								} else if(as.state == false && as.failure == false){
						...
						if(thescrape != ""){
							if(scrapeDS.get(thescrape) != null){
								AsynchScrape as = new AsynchScrape(scrapeDS.get(thescrape), Bukkit.getScheduler());
								as.failure = false;
								as.state = false;
								Bukkit.getScheduler().runTaskAsynchronously(this, as);
			...
			if (args[0].equalsIgnoreCase("get")) {
				...
				if (args[1].equalsIgnoreCase("uuid")) {
					...
					if (args.length==2) {
						...
						if (player==null) {
							sender.sendMessage("§2[UUID§aProvider§2] Player not found");
							return false;
						}
					}
					...
					sender.sendMessage("§2[UUID§aProvider§2] " + player.getName()+"'s UUID is "+(uuid!=null?uuid.toString():"null"));
					return true;
				}
				...
				if (args[1].equalsIgnoreCase("name")) {
					...
					if(name==null) {
						sender.sendMessage("§2[UUID§aProvider§2] Couldn't find the name for "+args[2]);
						return false;
					}
					
					sender.sendMessage("§2[UUID§aProvider§2] " + args[2] + " is " + name + "'s UUID.");
					return true;
				}
			}
			...
			if(args[0].equalsIgnoreCase("confirm")){
				if (getServer().getConsoleSender() == sender) {
					...
					if(clearConfirm.containsKey(args[1]) && clearConfirm.get(args[1]) == sender.getName()){
						cacheClear();
			...
			if (args[0].equalsIgnoreCase("clearcache")) {
				if (getServer().getConsoleSender() == sender) {
					...
					sender.sendMessage("§2[UUID§aProvider§2] §aPlease enter the command §e/uuidprovider confirm " + conf +"§a to confirm deletion of ALL CACHED DATA.");
...
// Cache Manipulation Methods
	 ...
	 * @param name - Name that the UUID belongs to
...
// Cache status methods
	 ...
	 * 
	 * @param p - an player name to check for against the player name cache.
	 ...
	 * 
	 * @param uuid - a UUID to check for against the player uuid cache.
	 * @return true if player was found, false if not.
	 */
	 ...
	 * 
	 * @param p - a PlayerData record to check for against the player UUID cache.
	 * @return true if player was found with identical data, false if not found, or data mismatch.
	 */
	static private boolean isCached(PlayerData p){
		if(p.name == cachedPlayersUUID.get(p.uuid).name && p.uuid == cachedPlayersName.get(p.name).uuid ){
			return true;
		} else {
			return false;
		}
	...
	/**boolean isCached(OfflinePlayer)
	 * 
	 * @param p - an offline player to check for against the player name cache.
	 * @return true if player was found, false if not.
	 */
...
// Helper stuff
	...
	private static boolean cRemove(String name, UUID uuid){
		//since we don't want to remove from the datastore, removing from
		//the cache doesn't make sense. so...NO.
		return false;
